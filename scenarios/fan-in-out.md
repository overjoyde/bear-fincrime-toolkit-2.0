...
- An entity receiving from at least 6 distinct counterparties within a
  72-hour window.
- The account then forwarding at least 70% of the total received shortly
  thereafter (within approximately twice that window) rather than retaining
  it as would be expected of a genuine counterparty.
...
least `MIN_PASS_THROUGH_RATIO` (0.7 = 70%) of the total received. The
alert-prioritization score (60–100, see `pipelines/README.md`) is derived
from how far the sender count and
